...
def build_prompt(params: PromptParams, include_default_negative=True) -> PromptResult:
    p = params
    p.validate()
    if type(include_default_negative) is not bool:
    ...
    blocks = [
        ...
        ("MATERIAL", {"mixed": "Use appropriate model materials per object: painted resin, fine wood and thin metal; preserve their distinct surface responses.", "resin": "Primary modeled surfaces are cast resin with fine molded edges and scale-appropriate thickness.", "wood": "Primary modeled surfaces are fine-grained wood; keep grain and joinery proportionate to miniature size.", "metal": "Primary modeled surfaces are thin formed metal with precise joints and fine machined edges.", "paper": "Primary modeled surfaces are layered paper and card with precise folds and thin cut edges.", "clay": "Primary modeled surfaces are sculpted clay with fine tool marks and controlled contours."}[p.material] + " " + {"matte": "Matte finish on painted surfaces; diffuse reflection without flattening glass or bare metal.", "satin": "Satin finish on painted surfaces with restrained soft highlights; retain material-specific reflections.", "gloss": "Gloss finish on coated surfaces with controlled highlights; uncoated materials retain their own finish."}[p.finish]),
        ("DETAIL", {"clean": "Sparse, deliberate small props and clean readable forms; avoid clutter.", "balanced": "Moderate secondary props, fine joints and restrained surface variation; keep the main subject dominant.", "intricate": "Precisely constructed joints, layered small props and material-specific microtexture; detail density must not obscure the subject."}[p.detail] + " " + {"pristine": "Pristine newly made surfaces; no dirt, scratches, chips or weathering.", "subtle": "Restrained wear only at plausible contact points; no oversized dust or scratches.", "aged": "Localized faded paint, fine chips and accumulated wear appropriate to each material; no uniform damage layer."}[p.wear]),
        ("CAMERA", {"macro": "Close-focus macro photography with controlled perspective.", "tilt": "Tilt-shift optics with a deliberate focus plane through the main subject; avoid an arbitrary horizontal blur band.", "standard": "Natural standard-lens perspective at miniature working distance, without wide-angle distortion."}[p.lens] + " " + {"selective": "Selective depth of field: keep essential subject features sharp, with gradual optical falloff into the background.", "deep": "Extended depth of field across the modeled scene; retain readable foreground and background, without selective blur."}[p.focus]),
        ("COMPOSITION", {"eye": "Camera at the modeled subject's eye level.", "three_quarter": "Elevated three-quarter view revealing volume and miniature construction.", "top": "True overhead view with the camera perpendicular to the base."}[p.angle] + " " + {"center": "Center the primary subject with balanced surrounding space.", "thirds": "Place the primary subject near a rule-of-thirds intersection; keep its full defining silhouette inside frame.", "wide": "Show the entire modeled arrangement with breathing room around its outer edges."}[p.composition]),
        ("LIGHTING", {"soft": "Large diffused studio source relative to the model; gentle fill, localized contact shadows and controlled highlights.", "daylight": "Soft window daylight illuminating the physical model, with coherent directional shadows and neutral material colors.", "golden": "Warm low-angle key light with soft neutral fill; preserve surface colors and consistent shadow direction.", "night": "Tiny practical lights in the miniature with low-level fill; retain shadow detail, restrained bloom and believable light falloff."}[p.lighting]),
        ("BACKGROUND", {"table": "A physical diorama base rests on a tabletop; show a restrained base edge without distracting workshop clutter.", "seamless": "A clean seamless neutral studio background, without a visible tabletop edge or additional background props.", "environment": "Continue the modeled environment behind the subject at the same scale; no full-size landscape or distant atmospheric haze."}[p.background]),
        ("SCALE REFERENCE", {"none": "No external reference objects, human hands or fingers; communicate miniature scale through construction, textures and optics.", "coin": "Exactly one real-size plain unbranded coin near the frame edge, outside the modeled world; no readable inscriptions. Keep it secondary to the model.", "pencil": "Exactly one real-size unbranded pencil near the frame edge, outside the modeled world; no lettering. Keep it secondary to the model.", "ruler": "Exactly one real-size ruler fragment near the frame edge, outside the modeled world, with tick marks only and no numerals. Keep it secondary to the model."}[p.cue]),
    ]
    blocks[4:4] = character_blocks(p) + style_blocks(p)
    if p.medium == "video":
        blocks.append(("MOTION", f"Duration: {p.duration} seconds. One continuous shot. " + {"locked": "Locked camera with no pan, zoom, shake or cuts.", "push": "A slow, smooth short forward camera move; preserve the subject's framing.", "slide": "A slow, smooth short lateral camera move with gentle parallax.", "orbit": "A slow small-arc camera move around the subject, without a full rotation."}[p.movement] + " Subject action: " + (p.action.strip() if use_custom_action else "Follow the events described in SCENE in their stated order, preserving the specified participants and their actions. Do not substitute actions from an unrelated scene. If SCENE specifies no action, do not invent one.") + " Keep the camera instructions separate from subject motion: a locked camera does not freeze the subjects. Maintain geometry, scale, materials and lighting across every frame; allow entrances, exits and occlusion required by SCENE without spontaneous duplication. Use continuous physically coherent motion, with stable focus and no flicker."))
    if p.extra.strip():
        blocks.append(("ADDITIONAL DIRECTION", p.extra.strip()))
    defaults = []
    if include_default_negative:
        defaults.append("Life-size scene, inconsistent miniature scale, oversized surface texture, warped geometry, duplicated main subjects, floating props, unreadable subject, excessive blur, blown highlights, watermark, logo, readable lettering, full-size people or hands")
        if p.characters:
            defaults.append("extra featured characters, identity swaps, merged faces or bodies, unintended extra limbs, detached accessories, obscured featured faces")
        if p.secondary_style:
            defaults.append("unrelated style collage, incompatible anatomy, split-face style fusion")
        if p.wear == "pristine":
            defaults.append("dirt, scratches, chipped paint, weathering")
        if p.focus == "deep":
            defaults.append("shallow depth of field, selective blur")
        if p.medium == "video":
            defaults.append("temporal flicker, morphing, teleportation, frame-to-frame object changes, jump cuts, focus hunting")
    if p.negative.strip():
        defaults.append(p.negative.strip())
    negative = ", ".join(defaults)
    positive = "\n\n".join(f"{heading}\n{text}" for heading, text in blocks)
    combined = positive + ("\n\nAVOID\n" + negative if negative else "")
    notes = ["자유 입력은 원문 그대로 반영됩니다. 자동 번역이나 AI 의미 검증은 수행하지 않습니다."]
    if not include_default_negative:
        notes.append("기본 제외 조건을 사용하지 않았습니다. 사용자가 입력한 제외 조건만 적용됩니다.")
