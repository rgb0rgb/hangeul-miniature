...
class MotionTests(unittest.TestCase):
    def setUp(self):
        self.params = replace(PRESETS["철도 디오라마"], scene=SCENE, subject=SUBJECT, medium="video")
    ...
    def test_reported_stale_action_is_not_emitted(self):
        ...
        self.assertIn("Follow the events described in SCENE in their stated order", result.positive)

    def test_explicit_action_is_bound_to_scene_and_subject(self):
        action = "우주선 문이 열리고 우주인들이 내려온다."
        params = replace(self.params, action=action, action_source="custom", action_context=action_context(SCENE, SUBJECT))
    ...
    def test_old_v2_and_v3_files_do_not_reactivate_stale_action(self):
        for version in (2, 3):
            ...
            for key in ("action_source", "action_context"):
                values.pop(key)
            if version == 2:
                for key in ("primary_style", "secondary_style", "blend_mode", "secondary_weight", "characters"):
                    values.pop(key)
    ...
    def test_ui_scene_and_subject_edits_reset_custom_action(self):
        ...
        for field, replacement in (("scene", SCENE), ("subject", SUBJECT)):
            with self.subTest(field=field):
                ...
                widget = app.text_area(key=field) if field == "scene" else app.text_input(key=field)
